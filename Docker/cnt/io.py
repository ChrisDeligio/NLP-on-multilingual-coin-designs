...
class Database_Connection():
    """
        ...
        mysql_connection : str
            Connection to database using mysqlconnector

        Methods
        --------
        load_designs_from_db(self, table_name, column_list):
            Query to load the designs

        load_entities_from_db(self, table_name, column_list, columns_multi_entries=[], delimiter="", has_delimiter=False):
            Query to load entities from the nlp_lists
        
        preprocess_entities(self, entities)
            Apply preprocess to entities
        create_own_query(self, query)
            used in case user wants to send a own query --> no error detection
